...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textbf{vCPU context:} a vCPU may be holding a mutex or
      spinlock, or running with interrupts disabled, with other vCPUs
      waiting on it
...
\column{0.55\linewidth}
\begin{itemize}
  \item Every guest does proportional back-off
  \item The amount of vCPUs to use is driven by steal time
  \item The vCPUs each guest can safely use are denoted as
    \emph{preferred} CPUs
  \item Aim: sum of preferred CPUs $\le$ pCPUs, which achieves less
    vCPU preemption
\end{itemize}
\medskip
...
\begin{itemize}
  ...
  \item Maintains the design construct:\\
    preferred $\le$ online $\le$ possible
\end{itemize}
\medskip
...
\begin{itemize}
  ...
  \item If steal time is above the threshold, reduce preferred CPUs
    by one core. Core granularity is used because some hypervisors
    (e.g.\ PowerVM) schedule at core granularity -- this yields the
    best benefit
...
\begin{itemize}
  \item \texttt{/sys/devices/system/cpu/preferred} -- current
    preferred CPUs
  \item \texttt{STEAL\_MONITOR} sched feature (disabled by default)
  \item Debugfs knobs for thresholds and sampling period
\end{itemize}
\begin{verbatim}
...
\begin{itemize}
  \item Augment the generic policy with arch-specific hints, like
    power entitlements or s390 polarization. Arch-specific hooks in
    the steal-time monitor code?
  \item Interaction with \texttt{irqbalance}: make it preferred-mask-aware, or require it to be disabled?
  \item Is there any !SMT system interested in this?
  \item NUMA-awareness to avoid cross-node penalties
  \item \texttt{sched\_ext}?
  \item How to avoid oscillations in complex overcommit cases?
  \item Should steal time be computed based on online CPUs or
    preferred CPUs?
\end{itemize}
\end{frame}
