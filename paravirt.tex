...
\begin{itemize}
  ...
  \end{itemize}
  \item \textbf{Design constraint:} the hypervisor is opaque and
    non-cooperative -- no new ABI, no host-side changes, no
    inter-guest channel (e.g.\ PR/SM, PowerVM)
\end{itemize}
...
\begin{itemize}
  \item \texttt{/sys/devices/system/cpu/preferred} -- current
    preferred CPUs
  \item \texttt{STEAL\_MONITOR} sched feature -- per-architecture
    opt-in; off until the arch enables it
