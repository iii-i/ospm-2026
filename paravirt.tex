...
\begin{itemize}
  ...
  \end{itemize}
  \item \textbf{Design constraint:} support different hypervisors,
      which means no guest-to-host or inter-guest channels
\end{itemize}
...
\begin{itemize}
  ...
  \item When \texttt{irqbalance} is off (rare), should the kernel
    push IRQs off non-preferred CPUs?
...
\begin{frame}{Extras: avoiding host under-utilization and oscillations}
\begin{itemize}
  \item \textbf{Goals:} avoid host under-utilization, avoid
    oscillations
  \item \textbf{Implemented today:} slow shrink, slow grow
  \item \textbf{Ideas:}
  \begin{itemize}
    \item Smooth the steal time signal with an EWMA filter -- same
      as PELT
    \item PI(D) controller on the steal time signal
  \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Extras: fairness without coordination}
\begin{itemize}
...
\begin{frame}{Extras: is this SMT-specific?}
\begin{itemize}
...
\begin{frame}{Extras: relative steal time}
\begin{itemize}
...
\begin{frame}{Extras: alternative push task mechanisms}
\begin{itemize}
  \item Vincent Guittot's CFS push-task mechanism
  \item {\small\href{https://lore.kernel.org/all/20251202181242.1536213-1-vincent.guittot@linaro.org/}{[PATCH
    ...
    cases}}
  \item Push all tasks off a CPU
  \item Reuse the CPU-offline task pushing code
\end{itemize}
\end{frame}
